...
//repeat until end of card:
    ...
    {
        //stack or heap?
        //stack if you know exactly how much data you need to allocate before compile time and it is not too big.
        //BYTE buffer[512];
        ...
        if ((buffer[0] == 0xff) && (buffer[1] == 0xd8) && (buffer[2] == 0xff) && ((buffer[3] & 0xf0) == 0xe0))  
        {
            ...
            if (jpeg_count == 0)   
            {
                ...
                FILE* img = fopen(name, "w");    //open filename ###.jpg //fopen is what makes the file (as long as there is no existing file with its name.)
                if (img == NULL)
                {
...
//reach end of memory card, close remaining files
    fclose(mcfile);
        ...
        else if (jpeg_count != 0) //if not start of new JPEG
        {
            FILE *img = fopen(name, "a");
            //keep writing already found JPEG
